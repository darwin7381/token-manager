...
## ❌ 錯誤 #14: 審計日誌功能導致主要操作失敗（2025-11-08）

### 嚴重程度
🔴 **極度嚴重** - 導致所有 CRUD 操作返回 500 錯誤，但資源實際已創建，造成用戶困惑

### 問題描述

在改進 Dashboard 最近活動記錄時，添加了審計日誌功能來記錄操作者 email。但由於多個問題疊加，導致審計日誌記錄失敗，進而讓整個 CRUD 操作返回 500 錯誤，儘管資源實際上已成功創建。
...
1. **改進 Dashboard 時添加了審計日誌的 email 記錄**
   - 原本審計日誌只記錄基本資訊
   - 為了顯示操作者，添加了 `created_by_email` 欄位
   - 但實施時有多個錯誤

2. **查詢缺少必要欄位**
   - 刪除路由時只查詢 `path`
   - 但審計日誌需要 `name`
   - 導致 `route['name']` KeyError

3. **Clerk SDK 物件未轉換**
   - `user.email_addresses` 是 Clerk SDK 物件
   - 直接傳遞給 user_data
   - 後續存取時可能出錯

4. **不安全的陣列存取**
   - `[{}][0]` 如果陣列為空會 IndexError
   - 沒有防禦性檢查
