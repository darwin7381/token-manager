...
## ❌ 錯誤 #14: Clerk SDK 物件未轉換導致 500 錯誤及 CORS 假象（2025-11-08）

### 嚴重程度
🔴 **極度嚴重** - 導致所有 CRUD 操作失敗，瀏覽器顯示 CORS 錯誤但實際是後端崩潰

### 問題描述

在 `verify_clerk_token` 中直接將 Clerk SDK 的物件（如 `user.email_addresses`）傳遞給返回值，導致後續代碼嘗試存取時出現類型錯誤或 IndexError，造成後端返回 500 錯誤。**更嚴重的是，瀏覽器會顯示 CORS 錯誤，誤導診斷方向。**

### 錯誤代碼

**核心問題：Clerk SDK 物件未轉換**

```python
# backend/clerk_auth.py - ❌ 核心錯誤
...
# ← user.email_addresses 不是普通 list！
...
```
瀏覽器 Console（誤導！）:
  ❌ Access to fetch at 'https://tapi.blocktempo.ai/api/routes' 
     from origin 'http://localhost:5173' 
     has been blocked by CORS policy: 
     No 'Access-Control-Allow-Origin' header is present
  ❌ POST /api/routes net::ERR_FAILED 500 (Internal Server Error)
  
診斷陷阱:
  → 看到 CORS 錯誤
  → 以為是 CORS 設置問題
  → 檢查 CORS middleware（實際上沒問題）
  → 浪費大量時間在錯誤的方向
...
實際真相:
  ...
  ✅ 資源已創建到資料庫
  ✅ 已同步到 Cloudflare KV
  ❌ 後端 500 錯誤 → 響應不完整 → CORS headers 缺失
  ❌ 瀏覽器誤以為是 CORS 問題
```

### 根本原因

**Clerk SDK 返回的物件不是 Python 原生類型，直接使用會導致後端崩潰和 CORS 假象。**

```python
# clerk_auth.py 的錯誤
user = clerk_client.users.get(user_id)  # Clerk SDK User 物件
return {
    "email_addresses": user.email_addresses  # ← 這不是 list！是 Clerk 物件！
}

# main.py 嘗試使用
user["email_addresses"][0]  # ← 類型錯誤或 IndexError
→ 拋出 500 錯誤
→ 響應不完整
→ 瀏覽器看不到 CORS headers
→ 顯示 "CORS policy blocked"（誤導！）
```

**關鍵認知：**
- ✅ 後端崩潰（500）會導致瀏覽器顯示 CORS 錯誤
- ❌ CORS 錯誤不等於 CORS 設置有問題
- ✅ 必須先看 Status code，再看 CORS

**次要問題：**查詢缺少欄位、陣列存取未檢查空值（簡要帶過）
...
await log_audit("delete", "route", route_id, {
    "name": route['name'],  # ✅ 有查詢，不會 KeyError
    "path": route['path'],
})
```

**階段 3: main.py - 安全存取 email**
...
1. **查詢時要包含所有後續需要的欄位**
2. **審計日誌失敗不應該讓整個操作失敗**
3. **SDK 物件必須轉換為 Python 原生類型**
4. **陣列存取要有防禦性檢查**
5. **新功能可能引入新錯誤** - 需要全面測試

### 影響範圍

**受影響的所有 CRUD 操作**（審計日誌記錄時失敗）：
- ❌ 創建 Token - email 陣列存取問題
- ❌ 更新 Token - email 陣列存取問題
- ❌ 刪除 Token - email 陣列存取問題
- ❌ 創建 Route - email 陣列存取問題 ← **fail to fetch 的根源！**
- ❌ 更新 Route - email 陣列存取問題
- ❌ 刪除 Route - 缺少 name 欄位 + email 問題 ← **刪除路由 fail to fetch！**

### 診斷過程（含誤判）

1. **用戶報告**: 創建路由顯示 "Failed to fetch"
2. **觀察**: 路由實際已創建（第二次說 already exists，KV 有資料）
3. **初步判斷**: 創建成功但返回失敗 → 可能是返回階段出錯
4. **誤判 1**: 看到 CORS 錯誤，以為是 CORS 設置問題 ❌
   - 調整 CORS 中間件順序（這個其實是對的，但不是根本原因）
5. **誤判 2**: 以為是 Vite proxy 硬編碼問題 ❌
   - 移除 proxy（已 revert，proxy 其實沒問題）
6. **誤判 3**: 以為是 KV 同步失敗 ❌
   - 改為非阻塞（已 revert，KV 同步沒問題）
7. **定位真因**: 檢查 Network tab，確認是 500 錯誤
8. **逐步排查**:
   - 檢查 sync_routes_to_kv JSONB 處理
   - 檢查 log_audit 參數
   - 發現 email_addresses 存取和 name 缺失問題
9. **全面修復**: 
   - clerk_auth.py 轉換 email_addresses
   - main.py 安全存取 email（6 處）
   - delete_route 查詢 name 欄位

### 修復的檔案

1. `backend/clerk_auth.py` - 轉換 email_addresses 為純 Python list
2. `backend/main.py` - 6 處安全存取 email + 1 處查詢補充 name 欄位
3. `backend/main.py` - sync_routes_to_kv JSONB 安全處理
...
3. **CORS 錯誤可能是表象，不是根因**
   - 後端 500 錯誤 → 響應不完整 → CORS headers 缺失
   - 瀏覽器顯示 CORS 錯誤，但真正原因是後端崩潰
   - **必須看 Network tab 的 Status code！**

4. **診斷時不要猜測，要看證據**
   - 本地/遠端都有 CORS 錯誤 → 不是環境問題
   - 操作成功但返回失敗 → 問題在返回前的某個步驟
   - 要完整追蹤整個流程，不要只看局部
