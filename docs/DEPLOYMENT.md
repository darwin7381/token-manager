...
## 7. 首次部署特殊處理：KV 反向同步機制

### ⚠️ 重要：理解這個機制

**問題場景**：
本地開發時創建的 Token/Routes 會同步到 Cloudflare KV（全局共用），但首次部署生產環境時 PostgreSQL 是空的：
- Worker 可以驗證這些 Token（從 KV 讀取）✅
- 前端管理界面看不到（從 PostgreSQL 讀取）❌
- 造成「幽靈 Token」無法管理

### 自動同步流程

**執行時機**：每次應用啟動時（`database.py` 的 `init_system_teams()`）
...
創建 core-team
  ↓
執行 sync_missing_from_kv()
  ├─→ 1. 同步 Tokens
  │     ├─ 列出 KV 中所有 token:* keys（支援分頁）
  │     ├─ 對比 PostgreSQL 的 token_hash
  │     ├─ 找出缺失的 Token
  │     ├─ 對每個缺失的 Token：
  │     │   ├─ 從 KV 讀取 Token 數據
  │     │   ├─ 確保團隊存在（_ensure_team_from_clerk）
  │     │   │   ├─ PostgreSQL 有？✅ 使用
  │     │   │   ├─ Clerk 有？查詢所有用戶的 teamRoles
  │     │   │   │   ├─ 找到成員 → 同步團隊資訊到 PostgreSQL
  │     │   │   │   └─ 沒找到 → 使用 core-team
  │     │   └─ INSERT INTO tokens ... ON CONFLICT DO NOTHING
  │     └─ 顯示統計：X 個導入，Y 個跳過
  │
  └─→ 2. 同步 Routes
        ├─ 從 KV 讀取 routes key（所有路由的 JSON）
        ├─ 對比 PostgreSQL 的 path
        ├─ 對每個缺失的 Route：
        │   ├─ 解析新舊格式（向後兼容）
        │   ├─ 提取 url, tags, auth 配置
        │   └─ INSERT INTO routes ... ON CONFLICT DO NOTHING
        └─ 顯示統計
```
...
3. **錯誤容錯**：
   - 個別 Token/Route 導入失敗：跳過，繼續
   - 整個同步失敗：catch，服務仍正常啟動
   - 不會因為同步失敗而阻止服務啟動
...
- `token_encrypted = NULL`（⚠️ 無法使用「複製」功能）
- `description = "從 Cloudflare KV 自動導入 (2025-11-08 16:30)"`

**Routes**：
- `name = "Imported: /api/openai"`
- `description = "從 Cloudflare KV 自動導入 (2025-11-08 16:30)"`

### 何時會執行

- ✅ 首次部署（PostgreSQL 空的）
- ✅ 之後每次啟動（但因為 ON CONFLICT，不會重複導入）
- ❌ 使用 dummy credentials 時跳過

### 潛在風險與注意事項

⚠️ **風險 #1**：KV 中的 Token 沒有 `token_encrypted`
- 導入後無法使用「複製」功能
- 解決：提示用戶重新創建 Token

⚠️ **風險 #2**：團隊資訊不完整
- KV 只有 team_id，沒有團隊名稱
- 解決：從 Clerk 同步真實團隊，找不到用 core-team

⚠️ **風險 #3**：Clerk API 返回格式變化
- 不同 SDK 版本可能返回不同格式
- 解決：同時支援 list 和 .data 格式
